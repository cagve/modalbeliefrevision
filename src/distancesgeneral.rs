use std::{cmp::{Ordering, self}, collections::HashSet, fmt, usize};
...
fn directed_hausdorff_distance(model1: &Vec<String>, model2:&Vec<String>) -> usize {
    model1.iter().map(|p1| {
        model2.iter().map(|p2| hamming_distance(p1, p2)).fold(usize::MAX, cmp::min)
    }).fold(usize::MIN, cmp::max)
}

fn hausdorff_distance(model1: &Vec<String>, model2:&Vec<String>) -> usize {
    let dhd1 = directed_hausdorff_distance(model1, model2);
    let dhd2 = directed_hausdorff_distance(model2, model1);
    usize::max(dhd1, dhd2)
}
...
pub fn distance_pointed_to_pointed(pointed1: &S5PointedModel, pointed2: &S5PointedModel) -> Lexicographic{
    ...
    let distance_model = hausdorff_distance(&m1, &m2);
